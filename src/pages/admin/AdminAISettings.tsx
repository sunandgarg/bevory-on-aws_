...
import { Save, Loader2, Brain, ShieldCheck, TestTube } from "lucide-react";
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data?.value) {
        ...
        setSettings({
          default_provider: val.default_provider || DEFAULT_SETTINGS.default_provider,
          openai_model: val.openai_model || DEFAULT_SETTINGS.openai_model,
          claude_model: val.claude_model || DEFAULT_SETTINGS.claude_model,
          perplexity_model: val.perplexity_model || DEFAULT_SETTINGS.perplexity_model,
        });
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            API keys are read only from Supabase Edge Function secrets. They are never stored in the database or returned to this browser.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <Label>OpenAI model</Label>
            <Select value={settings.openai_model} onValueChange={(v) => setSettings(s => ({ ...s, openai_model: v }))}>
          ...
          <div className="space-y-2">
            ...
            <Select value={settings.claude_model} onValueChange={(v) => setSettings(s => ({ ...s, claude_model: v }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={settings.perplexity_model} onValueChange={(v) => setSettings(s => ({ ...s, perplexity_model: v }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
